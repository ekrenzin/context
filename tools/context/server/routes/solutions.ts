...
export function registerSolutionRoutes(app: FastifyInstance, root: string): void {
  ...
  app.post<{ Params: { id: string } }>(
    "/api/solutions/:id/rebuild",
    async (req, reply) => {
      const sol = getSolution(req.params.id);
      if (!sol) return reply.code(404).send({ error: "Solution not found" });

      await deactivateSolution(req.params.id);
      updateStatus(req.params.id, "building");

      const result = await buildSolution({
        problem: sol.problem,
        name: sol.name,
        projectId: sol.project_id ?? undefined,
      });
      ...
      viewCache.delete(req.params.id);

      updateSolution(req.params.id, {
        description: result.description,
        proposal: result.proposal,
        components: result.components,
      });
      ...
      try {
        await activateSolution(req.params.id, dir, result.components, cmdCenterDir);
        updateStatus(req.params.id, "active");
      } catch (err) {
        const msg = err instanceof Error ? err.message : String(err);
        updateStatus(req.params.id, "error");
        return { ...getSolution(req.params.id), startError: msg };
      }
