...
function notifyExitAndCleanup(code) {
  for (const c of clients) {
    try { c.write(JSON.stringify({ type: "exit", code: code ?? -1 }) + "\n"); } catch {}
  }
  cleanup();
  process.exit(0);
}

process.on("SIGTERM", () => notifyExitAndCleanup(exitCode));
process.on("SIGINT", () => notifyExitAndCleanup(exitCode));
