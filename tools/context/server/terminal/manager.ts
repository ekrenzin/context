...
const tapConnections = new Map<string, SocketPty>();
...
export function registerTap(id: string, pty: SocketPty): void {
  tapConnections.set(id, pty);
}

export function killSession(id: string): boolean {
  const session = sessions.get(id);
  if (!session) return false;
  // Disconnect the tap before killing so it doesn't hold the socket open
  const tap = tapConnections.get(id);
  if (tap) {
    tap.disconnect();
    tapConnections.delete(id);
  }
