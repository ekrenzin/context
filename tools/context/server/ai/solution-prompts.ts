...
Respond with ONLY this JSON (no markdown fences):
{
  ...
  "description": "One-sentence summary of what this solution provides",
  "proposal": "2-4 sentence design rationale: what approach was chosen, why these components, and what the user should expect"
}`;
}
...
- Register all routes at the root level (e.g. /deadlines, /items) -- do NOT use an /api prefix.
...
export function solutionViewPrompt(
  ...
  const apiHint = servicePort
    ? `Fetch data from http://localhost:${servicePort}/... (e.g. http://localhost:${servicePort}/deadlines). Do NOT add an /api prefix -- the service routes are registered at the root (e.g. /health, /deadlines). A proxy layer rewrites these URLs at runtime.`
