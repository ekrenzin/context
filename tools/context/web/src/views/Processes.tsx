...
function ServerOriginCard({ info }: { info: ServerProcess | null }) {
  if (!info) {
    return (
      <Card>
        <CardContent sx={{ py: 1.5, "&:last-child": { pb: 1.5 } }}>
          <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
            ...
            <Typography variant="body2" fontWeight={600}>Server Process</Typography>
          </Box>
  ...
  const startCmd = info.argv.length > 1
    ? info.argv.slice(1).map((a) => a.split("/").pop()).join(" ")
    : info.argv[0];
    ...
    <Card>
      <CardContent sx={{ py: 1.5, "&:last-child": { pb: 1.5 } }}>
        <Box sx={{ display: "flex", alignItems: "center", gap: 1, mb: 1 }}>
          <TerminalIcon color="primary" />
          <Typography variant="body2" fontWeight={600}>Server Process</Typography>
          <Chip label={`PID ${info.pid}`} size="small" variant="outlined" sx={{ fontSize: "0.65rem" }} />
          <Chip label={formatUptime(info.uptimeSec)} size="small" color="success" variant="outlined" sx={{ fontSize: "0.65rem" }} />
        </Box>
        <Stack spacing={0.25}>
          <Typography variant="caption" color="text.secondary">
          ...
          <Typography variant="caption" color="text.secondary" noWrap>
            Command: <code>{startCmd}</code>
          </Typography>
          <Typography variant="caption" color="text.secondary" noWrap>
            CWD: <code>{info.cwd}</code>
          </Typography>
          {info.tty && (
            <Typography variant="caption" color="text.secondary">
              Terminal: {info.tty}
          ...
          <Typography variant="caption" color="text.secondary">
            Node {info.nodeVersion} | Parent PID {info.ppid}
          </Typography>
        </Stack>
      </CardContent>
    </Card>
...
function AutoCommitCard({ status }: { status: AutoCommitStatus | null }) {
  const triggerPush = useCallback(() => {
    fetch("/api/auto-commit/trigger", { method: "POST" }).catch(() => {});
  }, []);
      ...
      <Card>
        <CardContent>
          <Box sx={{ display: "flex", alignItems: "center", gap: 1, mb: 1 }}>
            ...
            <Typography variant="h6" fontWeight={600}>Auto Commit</Typography>
          </Box>
          <Typography variant="body2" color="text.secondary">Waiting for data...</Typography>
        </CardContent>
      </Card>
      ...
      <CardContent>
        <Box sx={{ display: "flex", alignItems: "center", gap: 1, mb: 1.5 }}>
          ...
          <Chip
            label={push.state}
            size="small"
            color={push.state === "running" ? "warning" : push.state === "disabled" ? "error" : "default"}
            variant="outlined"
          />
          {save.commits > 0 && (
            <Chip
              label={`${save.commits} local`}
              size="small"
              color="info"
              variant="outlined"
              sx={{ fontSize: "0.65rem" }}
          ...
          {push.lastResult !== "none" && (
            <Chip
              label={push.lastResult}
              size="small"
              color={statusColor(push.lastResult === "success" ? "completed" : push.lastResult)}
              variant="outlined"
            />
          )}
          <Box sx={{ flex: 1 }} />
          <Tooltip title="Squash + push now">
            <IconButton size="small" onClick={triggerPush} color="primary" disabled={push.state === "running"}>
              <PlayArrowIcon fontSize="small" />
            </IconButton>
          </Tooltip>
        </Box>

        {push.state === "running" && <LinearProgress sx={{ mb: 1.5, borderRadius: 2 }} />}

        <Stack spacing={0.5}>
          <Stack direction="row" spacing={2}>
            <Typography variant="caption" color="text.secondary">
              Local save: every 30s
            </Typography>
            <Typography variant="caption" color="text.secondary">
              Last save: {relativeTime(save.lastAt)}
            </Typography>
          </Stack>
          {push.lastMessage && (
          ...
          {push.lastCommitSha && (
            <Typography variant="caption" color="text.secondary">
              Last pushed: <code>{push.lastCommitSha}</code>
            </Typography>
          )}
          <Stack direction="row" spacing={2}>
            <Typography variant="caption" color="text.secondary">
              Last push: {relativeTime(push.lastRunAt)}
            </Typography>
            <Typography variant="caption" color="text.secondary">
              Next push: {relativeTime(push.nextRunAt)}
            </Typography>
          </Stack>
        </Stack>
      </CardContent>
