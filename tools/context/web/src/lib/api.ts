...
export const api = {
  ...
  // Local AI
  localAiPrompt: (prompt: string, opts?: { maxTokens?: number; temperature?: number }) =>
    postJson<{ ok: boolean; response?: string; error?: string }>(
      "/api/local-ai/prompt",
      { prompt, ...opts },
    ),

  localAiStatus: () =>
    get<{ status: string; model: string | null; ollama: boolean }>(
      "/api/local-ai/status",
    ),
  ...
  cliToolsInstall: (tool: "claude" | "codex" | "cloudflared") =>
    postJson<{ success: boolean; error?: string }>("/api/cli-tools/install", { tool }),
  ...
  // Proposals
  listProposals: () => get<ProposalSummary[]>("/api/proposals"),

  createProposal: (description: string) =>
    postJson<{ sessionId: string }>("/api/proposals", { description }),

  getProposal: (slug: string) => get<ProposalDetail>(`/api/proposals/${slug}`),

  updateTaskStatus: (slug: string, taskNum: number, status: string) =>
    patchJson<{ ok: boolean }>(`/api/proposals/${slug}/tasks/${taskNum}`, { status }),

  buildProposal: (slug: string, opts?: { task?: number; agent?: string }) =>
    postJson<{ sessionId: string; slug: string; task: number | null; agent: string; promptPath: string }>(
      `/api/proposals/${slug}/build`,
      opts ?? {},
    ),

  editProposal: (slug: string, opts?: { task?: number }) =>
    postJson<{ sessionId: string; slug: string; task: number | null; promptPath: string }>(
      `/api/proposals/${slug}/edit`,
      opts ?? {},
    ),

  deleteProposal: (slug: string) =>
    fetch(`/api/proposals/${slug}`, { method: "DELETE" }).then(async (r) => {
